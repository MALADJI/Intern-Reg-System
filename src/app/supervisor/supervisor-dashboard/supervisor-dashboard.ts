...
export class SupervisorDashboard implements OnInit, OnDestroy {
    ...
    this.subscriptions.add(
      this.route.queryParams.subscribe(params => {
        if (params['section']) {
          ...
          if (['overview', 'interns', 'leave-requests', 'attendance', 'reports', 'profile'].includes(section)) {
  ...
  // ===== Navigation =====
  activeSection: 'overview' | 'interns' | 'Intern Leave status' | 'history' | 'reports' | 'Locations' | 'profile' = 'overview';

  showSection(section: string) {
    const validSections: Array<'overview' | 'interns' | 'Intern Leave status' | 'history' | 'reports' | 'Locations'> = [
      'overview', 'interns', 'Intern Leave status', 'history', 'reports', 'Locations'
    ];
