...
export class SuperAdminDashboard implements OnInit, OnDestroy {
  ...
  showBulkImportAdminModal: boolean = false;
  isBulkImportingAdmins: boolean = false;
  adminCsvContent: string = '';
  bulkAdminImportResult: any = null;
  ...
  editDepartmentForm: { name: string; surname: string; email: string; departmentId?: number | null } = {
    name: '',
    surname: '',
    email: '',
  ...
  createAdmin(): void {
    // Validate form
    if (!this.adminForm.name || !this.adminForm.surname || !this.adminForm.email || !this.adminForm.staffNumber) {
      Swal.fire({
        icon: 'warning',
        title: 'Validation Error',
        text: 'Please fill in all required fields (Name, Surname, Email, and Staff Number).'
      });
      return;
    }
    ...
    const adminData: any = {
      ...
      staffNumber: this.adminForm.staffNumber,
    ...
    this.api.post<any>('super-admin/admins', adminData).subscribe({
      next: (response) => {
        ...
        this.adminPassword = response.password || ''; // Use password from backend if available
  ...
  openBulkImportAdminModal(): void {
    this.showBulkImportAdminModal = true;
    this.adminCsvContent = '';
    this.bulkAdminImportResult = null;
  }

  closeBulkImportAdminModal(): void {
    this.showBulkImportAdminModal = false;
    if (this.bulkAdminImportResult && this.bulkAdminImportResult.createdCount > 0) {
      this.loadAdmins(); // Refresh list if any were created
    }
  }

  onAdminCsvFileSelected(event: any): void {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e: any) => {
        this.adminCsvContent = e.target.result;
      };
      reader.readAsText(file);
    }
  }

  processBulkAdminImport(): void {
    if (!this.adminCsvContent || !this.adminCsvContent.trim()) {
      Swal.fire('Error', 'Please provide CSV content or upload a file.', 'error');
      return;
    }

    this.isBulkImportingAdmins = true;
    this.bulkAdminImportResult = null;

    this.api.post<any>('super-admin/admins/bulk-csv', { csvContent: this.adminCsvContent }).subscribe({
      next: (response) => {
        this.isBulkImportingAdmins = false;
        this.bulkAdminImportResult = response;

        if (response.createdCount > 0) {
          Swal.fire({
            icon: 'success',
            title: 'Import Completed',
            text: `Successfully created ${response.createdCount} admins.`,
          ...
          Swal.fire({
            ...
            text: 'No new admins were created. Check the results below.',
            confirmButtonText: 'OK'
          });
        }
      },
      error: (error) => {
        ...
        const errorMessage = error.error?.message || error.error?.error || 'An error occurred during bulk import.';
...
Welcome to the Intern Register System! Your admin profile has been created successfully.
...
Super Admin`;
  ...
  deleteAllAdmins(): void {
    Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="mb-3 text-danger">Are you sure you want to delete <strong>ALL</strong> admins?</p>
          <div class="alert alert-danger mb-0">
            <i class="bi bi-exclamation-triangle-fill me-2"></i>
            <strong>CRITICAL WARNING:</strong> This will permanently remove all administrative accounts from the system. 
            <p class="mt-2 mb-0 small text-dark"><i class="bi bi-shield-check text-primary me-1"></i>Note: Super Admin accounts will be automatically preserved for safety.</p>
          </div>
        </div>
      `,
      icon: 'warning',
      ...
      preConfirm: () => {
        return new Promise((resolve, reject) => {
          this.api.delete<any>('super-admin/admins/delete-all').subscribe({
            next: (response) => resolve(response),
            error: (err) => {
              const errorMessage = err?.error?.message || err?.message || 'Failed to delete all admins.';
              reject(errorMessage);
      ...
      if (result.isConfirmed) {
        // Refetch admins to update the table
        this.loadAdmins();

        Swal.fire({
          icon: 'success',
          title: 'All Admins Deleted',
          text: result.value.message || 'All non-Super Admin accounts have been removed.',
          timer: 3000,
          showConfirmButton: false
        });
        this.cdr.detectChanges();
