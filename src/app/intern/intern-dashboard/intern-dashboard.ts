...
export class InternDashboard implements OnInit, AfterViewInit, OnDestroy {
  ...
  async downloadReportPDF(): Promise<void> {
    ...
    // Load and add Univen logo from assets asynchronously
    try {
      await new Promise<void>((resolve) => {
        const img = new Image();
        img.src = 'assets/univen-logo-2.png';
        img.onload = () => {
          doc.addImage(img, 'PNG', 250, 8, 30, 15);
          resolve();
        };
        img.onerror = () => {
          // Fallback: check DOM just in case
          const logoImg = document.querySelector('.user-logo') as HTMLImageElement;
          if (logoImg && logoImg.complete) {
            doc.addImage(logoImg, 'PNG', 250, 8, 30, 15);
          }
          resolve();
        };
      });
    } catch (e) {
      console.warn('Could not add Univen logo to PDF:', e);
    }
