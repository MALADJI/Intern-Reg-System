...
function getWebSocketUrl(): string {
  ...
  const backendHost = hostname === 'localhost' || hostname === '127.0.0.1'
    ? 'localhost'
    : hostname;
  return `${protocol}//${backendHost}:8082/ws`;
}
...
export class WebSocketService {
  ...
  public logUpdates$ = new Subject<WebSocketMessage>();
    ...
    try {
      ...
      this.stompClient = new Client({
        ...
        onStompError: (frame) => {
          ...
          // Relying on stompjs built-in reconnectDelay
        },
        onWebSocketClose: () => {
  ...
  private subscribeToChannels(): void {
    ...
    this.stompClient.subscribe('/topic/logs', (message: IMessage) => {
      try {
        const data: WebSocketMessage = JSON.parse(message.body);
        console.log('📨 Received activity log update:', data);
        this.logUpdates$.next(data);
        this.messageSubject.next(data);
      } catch (error) {
        console.error('Error parsing activity log message:', error);
      }
    });

    // Super Admin Logs (only if user is Super Admin)
    const authService = this.getAuthService();
    if ((authService as any).getUserRole && (authService as any).getUserRole() === 'SUPER_ADMIN') {
      this.stompClient.subscribe('/topic/superadmin-logs', (message: IMessage) => {
        try {
          const data: WebSocketMessage = JSON.parse(message.body);
          console.log('📨 Received Super Admin activity log:', data);
          this.logUpdates$.next(data);
          this.messageSubject.next(data);
        } catch (error) {
          console.error('Error parsing superadmin log message:', error);
        }
      });
    }
