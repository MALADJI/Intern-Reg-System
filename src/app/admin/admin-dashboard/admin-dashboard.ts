...
export class AdminDashboard implements OnInit, OnDestroy {
  ...
  openManageModal(item: any, type: string) {
    this.selectedManageItem = item;
    this.selectedManageType = type;
  }

  closeManageModal() {
    this.selectedManageItem = null;
    this.selectedManageType = '';
  }
  ...
  /**
   * Admin-only: Reset (clear) an intern's face registration.
   * After clearing, the intern can enroll a new face from the face-registration page.
   */
  resetInternFace(intern: any) {
    if (!intern.id) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'Intern ID is missing.' });
      return;
    }

    Swal.fire({
      title: '🔓 Reset Face Registration?',
      html: `<p>This will <strong>delete the registered face</strong> for <strong>${intern.name}</strong>.</p>
             <p class="text-danger mb-0"><small>The intern will need to re-register their face the next time they log in.</small></p>`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#dc3545',
      cancelButtonColor: '#6c757d',
      confirmButtonText: 'Yes, Reset Face',
      cancelButtonText: 'Cancel',
      reverseButtons: true
    }).then((result: SweetAlertResult) => {
      if (result.isConfirmed) {
        this.api.delete<any>(`interns/${intern.id}/face`).subscribe({
          next: (res) => {
            Swal.fire({
              icon: 'success',
              title: 'Face Reset!',
              text: `Face data for "${intern.name}" has been cleared. They can now re-enroll.`,
              timer: 3000,
              showConfirmButton: false
            });
          },
          error: (err) => {
            ...
            const msg = err.error?.message || err.error?.error || 'Failed to reset face data.';
  ...
  showBulkImportSupervisorModal: boolean = false;
  isBulkImportingSupervisors: boolean = false;
  supervisorCsvContent: string = '';
  bulkSupervisorImportResult: any = null;
  ...
  // --- Bulk CSV Import Methods ---

  openBulkImportSupervisorModal(): void {
    this.showBulkImportSupervisorModal = true;
    this.supervisorCsvContent = '';
    this.bulkSupervisorImportResult = null;
  }

  closeBulkImportSupervisorModal(): void {
    this.showBulkImportSupervisorModal = false;
    if (this.bulkSupervisorImportResult && this.bulkSupervisorImportResult.createdCount > 0) {
      this.loadSupervisors(); // Refresh list if any were created
    }
  }

  onSupervisorCsvFileSelected(event: any): void {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e: any) => {
        this.supervisorCsvContent = e.target.result;
      };
      reader.readAsText(file);
    }
  }

  processBulkSupervisorImport(): void {
    if (!this.supervisorCsvContent || !this.supervisorCsvContent.trim()) {
      Swal.fire('Error', 'Please provide CSV content or upload a file.', 'error');
      return;
    }

    this.isBulkImportingSupervisors = true;
    this.bulkSupervisorImportResult = null;

    this.api.post<any>('supervisors/bulk-csv', { csvContent: this.supervisorCsvContent }).subscribe({
      next: (response) => {
        this.isBulkImportingSupervisors = false;
        this.bulkSupervisorImportResult = response;

        if (response.createdCount > 0) {
          Swal.fire({
            icon: 'success',
            title: 'Import Completed',
            text: `Successfully created ${response.createdCount} supervisors.`,
          ...
          Swal.fire({
            ...
            text: 'No new supervisors were created. Check the results below.',
            confirmButtonText: 'OK'
          });
        }
      },
      error: (error) => {
        ...
        Swal.fire('Import Failed', error.error?.error || 'An error occurred during bulk import.', 'error');
      }
    });
  }
  ...
  updateSupervisorField(): void {
    ...
    // Validate form
    if (!this.editSupervisorForm.name || !this.editSupervisorForm.surname || !this.editSupervisorForm.email || !this.editSupervisorForm.staffNumber) {
      Swal.fire({
        icon: 'warning',
        title: 'Validation Error',
        text: 'Name, Surname, Email, and Staff Number are required.'
      });
      return;
    }
    ...
    const updateData: any = {
      ...
      staffNumber: this.editSupervisorForm.staffNumber.trim()
    };
  ...
  /**
   * Delete all supervisors (high-stakes action)
   */
  deleteAllSupervisors(): void {
    Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="mb-3">Are you absolutely sure? This will delete <strong>ALL</strong> supervisors and their user accounts from the system.</p>
          <div class="alert alert-danger mb-0 border-0 rounded-3 shadow-sm">
            <i class="bi bi-exclamation-octagon-fill me-2"></i>
            <strong>Critical Warning:</strong> This action is irreversible and will permanently wipe all supervisor profiles even from the database.
          </div>
        </div>
      `,
      icon: 'warning',
      ...
      preConfirm: () => {
        return this.api.delete<any>('supervisors/all').toPromise()
          .then(response => {
            return response;
          })
          .catch(error => {
            Swal.showValidationMessage(
              `Request failed: ${error.error?.message || error.message || 'Server error'}`
            );
          });
      },
      ...
      if (result.isConfirmed) {
        this.supervisors = []; // Clear list
        Swal.fire({
          icon: 'success',
          title: 'All Deleted!',
          text: result.value?.message || 'All supervisors have been removed from the database.',
          confirmButtonText: 'OK'
        });
        this.cdr.detectChanges();
      }
  ...
  createSupervisor(): void {
    // Validate form
    // Validate form
    if (!this.supervisorForm.name || !this.supervisorForm.surname || !this.supervisorForm.email || !this.supervisorForm.staffNumber || !this.supervisorForm.departmentId) {
      Swal.fire({
        icon: 'warning',
        title: 'Validation Error',
        text: 'Please fill in all required fields (Name, Surname, Email, Staff Number, and Department).'
      });
      return;
    }
    ...
    const supervisorData: any = {
      name: fullName,
      surname: this.supervisorForm.surname.trim(),
      email: this.supervisorForm.email,
      staffNumber: this.supervisorForm.staffNumber,
    ...
    this.supervisorService.createSupervisor(supervisorData).subscribe({
      next: (response) => {
        ...
        this.newlyCreatedSupervisor = {
          ...
          staffNumber: response.staffNumber || this.supervisorForm.staffNumber,
        ...
        // Store staff number temporarily for the invite message
        this.supervisorPassword = ''; // No local password
        this.lastInvitedSupervisorEmail = this.supervisorForm.email;
        const supervisorStaffNumber = this.supervisorForm.staffNumber;
  ...
  generateDefaultMessage(): void {
    ...
    // Message for newly created supervisor (using Univen credentials)
    this.inviteMessage = `Hello ${supervisorName},

Welcome to the Intern Register System! Your supervisor profile has been created successfully.

You can now log in using your standard Univen staff credentials:
...
Admin`;
  ...
  sendInviteEmail(): void {
    ...
    const inviteData: any = {
      ...
      sendToEmail: this.customSendToEmail,
